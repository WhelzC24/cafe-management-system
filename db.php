...
$db = getenv('DB_NAME') ?: 'web_system';
$primary_host = getenv('DB_HOST') ?: 'localhost';
$primary_user = getenv('DB_USER') ?: 'root';
$primary_pass = getenv('DB_PASS') !== false ? (string) getenv('DB_PASS') : '2005';
$primary_port = (int) (getenv('DB_PORT') ?: 3306);

mysqli_report(MYSQLI_REPORT_OFF);

$credential_candidates = [
    [$primary_host, $primary_user, $primary_pass, $primary_port, getenv('DB_SOCKET') ?: null],
    [$primary_host, 'root', '', $primary_port, null],
    [$primary_host, 'root', 'root', $primary_port, null],
    ['127.0.0.1', $primary_user, $primary_pass, $primary_port, null],
    ['localhost', $primary_user, $primary_pass, $primary_port, '/run/mysqld/mysqld.sock'],
    ['localhost', $primary_user, $primary_pass, $primary_port, '/var/run/mysqld/mysqld.sock'],
];

$conn = null;
foreach ($credential_candidates as $candidate) {
    [$host, $user, $pass, $port, $socket] = $candidate;
...
if (!$conn) {
    ...
    die('Database connection failed. Set DB_HOST, DB_USER, DB_PASS, DB_NAME, and optional DB_PORT/DB_SOCKET in your web server environment.');
}

// Keep MySQL TIMESTAMP conversion aligned with app timezone.
$tz = new DateTime('now', new DateTimeZone($app_timezone));
$offset = $tz->format('P'); // e.g. +08:00
mysqli_query($conn, "SET time_zone = '" . mysqli_real_escape_string($conn, $offset) . "'");

// Ensure must_change_password column exists
$col_check = mysqli_query($conn, "SELECT COUNT(*) AS total FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'must_change_password'");
if ($col_check) {
    $col_row = mysqli_fetch_assoc($col_check);
    mysqli_free_result($col_check);
    if ((int)($col_row['total'] ?? 0) === 0) {
        mysqli_query($conn, "ALTER TABLE users ADD COLUMN must_change_password TINYINT(1) NOT NULL DEFAULT 0 AFTER password");
    }
}
...
// Add image_url column to products if it doesn't exist (migration for existing installs)
$img_check = mysqli_query($conn, "SELECT COUNT(*) AS total FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'products' AND COLUMN_NAME = 'image_url'");
if ($img_check) {
    $img_row = mysqli_fetch_assoc($img_check);
    mysqli_free_result($img_check);
    if ((int)($img_row['total'] ?? 0) === 0) {
        mysqli_query($conn, "ALTER TABLE products ADD COLUMN image_url VARCHAR(500) DEFAULT NULL AFTER price");
    }
}
...
if ($product_count_result) {
    ...
    mysqli_free_result($product_count_result);

    if ((int)($product_count_row['total'] ?? 0) === 0) {
        $seed_products = [
        ...
        $insert_stmt = mysqli_prepare($conn, 'INSERT INTO products (name, category, description, price, image_url) VALUES (?, ?, ?, ?, ?)');
        if ($insert_stmt) {
            foreach ($seed_products as $p) {
                [$name, $category, $description, $price, $image_url] = $p;
                mysqli_stmt_bind_param($insert_stmt, 'sssds', $name, $category, $description, $price, $image_url);
